...
CREATE OR REPLACE FUNCTION embedefy_embeddings_table_create(
  ...
  input_model text,
  input_embeddings_table text DEFAULT NULL
)
RETURNS text AS $$
DECLARE
...
BEGIN
  ...
  ',
    ...
    input_table,
    embedding_dims,
    array_to_string(input_primary_keys, ', '),
    input_table, array_to_string(input_primary_keys, ', ')
  );
...
BEGIN
  ...
  -- Add the input column to the join condition
  IF array_length(select_query_join_condition, 1) > 0 THEN
    select_query_join_condition := array_append(select_query_join_condition, format('%I.column_name = %L', embeddings_table, input_column));
    select_query_join_condition := array_append(select_query_join_condition, format('%I.model_name = %L', embeddings_table, input_model));
  END IF;

  -- Add the input column to the where condition
  IF array_length(select_query_where_condition, 1) > 0 THEN
    -- For no we only support character varying columns
    select_query_where_condition := array_append(select_query_where_condition, format('%I.%I != ''''', input_table, input_column));
  END IF;
  ...
  ',
    ...
    array_to_string(select_query_where_condition, ' AND '),
    input_limit
  );
  -- Add the limit to the query
  IF input_limit > 0 THEN
    select_query := format('%s LIMIT %L', select_query, input_limit);
  END IF;
